...
if ( class_exists( 'Iconic_WIS_Core_Settings' ) ) {
	return;
}

/**
 * Iconic_WIS_Core_Settings.
 *
 * @class    Iconic_WIS_Core_Settings
 * @version  1.0.6
 */
class Iconic_WIS_Core_Settings {
	/**
	 * Single instance of the Iconic_WIS_Core_Settings object.
	 *
	 * @var Iconic_WIS_Core_Settings
	 */
	 ...
	 *
	 * @var Iconic_WIS_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_WIS_Core_Settings object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Iconic_WIS_Core_Settings
	 */
	...
	private function __construct() {
		...
		if ( defined( 'ICONIC_WIS_FILE' ) ) {
			add_filter( 'plugin_action_links_' . plugin_basename( ICONIC_WIS_FILE ), array( __CLASS__, 'plugin_action_links' ) );
		}
	}
	...
	public static function init() {
		...
		self::$settings_framework = new Iconic_WIS_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function get_plugin_slug() {
		return str_replace( '-', '_', 'iconic-wis' );
	}
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'iconic-wis-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'iconic-wis' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_url( self::get_docs_url() . $link['href'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=getting-started-links&utm_content=iconic-wis" target="_blank"><?php echo esc_html( $link['title'] ); ?></a>
				</li>
			<?php } ?>
		</ol>
		<?php
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'iconic-wis' ),
		);

		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_WIS_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'iconic-wis' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'license_field',
						'title'    => __( 'License Key', 'iconic-wis' ),
						'subtitle' => __( 'Activate your license to get the latest updates and support.', 'iconic-wis' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
						'id'       => 'account',
						'title'    => __( 'Your Account', 'iconic-wis' ),
						'subtitle' => __( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', 'iconic-wis' ),
						'type'     => 'custom',
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'iconic-wis' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'iconic-wis' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'iconic-wis' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'iconic-wis' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'iconic-wis' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_WIS_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Iconic_WIS_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'iconic-wis' ); ?></h3>
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['link'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=cross-sell&utm_content=iconic-wis" target="_blank"><?php echo wp_kses_post( $cross_sell['title']['rendered'] ); ?></a></h4>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', 'iconic-wis' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'iconic-wis' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=support-link&utm_content=iconic-wis', esc_html__( 'Submit Ticket', 'iconic-wis' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=documentation-link&utm_content=iconic-wis" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), esc_html__( 'Read Documentation', 'iconic-wis' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://my.iconicwp.com/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=account-link&utm_content=iconic-wis', esc_html__( 'Manage Your Account', 'iconic-wis' ) );
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'iconic-wis' ) . '">' . esc_html__( 'Settings', 'iconic-wis' ) . '</a>',
		);
